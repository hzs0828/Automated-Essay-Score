...
class Document:
    ...
    def __init__(self, gec_output):
        ...
        # print(self.__title)
        # print(gec_output["sent_nums"])
        # print(gec_output["sentece_0"])
...
class BertWorker:
    ...
    def input_fn_builder_file_path(self, file_path):
        def gen_asap_article():
            dataset = pd.read_csv(file_path,sep='\t',encoding='iso-8859-15')
...
def generate_xgboost_train_set(articles_id,
    ...
    with open(dataset_gec_path, encoding="utf-8") as fr:
        for line in fr:
            ...
            gec_output = json.loads(line_split[1].strip(),object_pairs_hook=OrderedDict)
            # print(gec_output)
...
if __name__ == "__main__":
    # 自己添加的代码　调用read_dataset_into_tfrecord函数
    # dataset_path＝sys_conf["data_dir"]
    # print("正在执行...")
    # read_dataset_into_tfrecord(sys_conf["data_dir"], BertWorker())
    # print("执行成功")
    ...
    articles_id = list(asap_dataset["essay_id"])
    articles_set = list(asap_dataset["essay_set"])
    domain1_scores = asap_dataset["domain1_score"]
    train_set_gec_result_path = './gec_result.txt'
    train_set_saved_path = './data/liujiawei/eilts_score/dataset/train_set_nogec.npz'
    generate_xgboost_train_set(articles_id,
                               articles_set,
                               domain1_scores,
                               train_set_gec_result_path,
                               train_set_saved_path)
    ...
    # bc = BertClient()
    # result = {}
    # prompt_npz = "./data/liujiawei/eilts_score/dataset/prompt.npz"
    # with open("./data/liujiawei/eilts_score/dataset/prompt", "r") as reader:
