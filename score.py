...
class OverallScorePredictor:
    ...
    # 原代码  ns_generate_type="no_negative" ps_generate_type="shuffle_all"
    def generate_asap_train_and_test_set(self,
                                         ps_generate_type="shuffle_all",
                                         ns_generate_type="both"):
        """ 生成asap的训练数据集合
    ...
    def train(self,
        ...
        for i in correspond_train_id_set:
            ...
            #     print("存在103467")
            if (i in features or (
                    i - 100000) in features) and i in basic_scores:
                temp_i = i
                if temp_i > 100000:
                    temp_i -= 100000
        ...
        for i in correspond_test_id_set:
            if (i in features or (
                ...
                if i in self.__ns_from_other_prompt_train or i in self.__ns_from_permuted_train:
                    ...
                    # print(articles_id)
                    # if i!=103467 and i!=118478:

                    # articles_id_1, articles_set, set_ids, handmark_scores = read_asap_dataset()
                    # print("1")
                    # if i in articles_id_1:
                    #     print(i)
                    #     test_handmark_normalized_scores.append(features[i][-1])
                    # else:
                    #     pass
        ...
        # hzs 修改 加入permuted 负样本
        xgb_rg.save_model(os.path.join(saved_model_dir, "osp.xgboost_prompt"))
        ...
        for i in range(len(correspond_test_id_set)):
            ...
            '''
            print("id:{}, basic:{}, predict:{}, handmark:{}".format(correspond_test_id_set[i],
                                                                                         basic_scores[
                                                                                             correspond_test_id_set[
                                                                                                 i]] * 10,
                                                                                         overall_score,
...
def main():
    ...
    if args.model == "osp":
        ...
        # hzs 修改
        xgboost_train_file_path = os.path.join(sys_conf["data_dir"], "train_set_nogec.npz")
